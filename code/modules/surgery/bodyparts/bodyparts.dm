...
/obj/item/bodypart/proc/receive_damage(brute = 0, burn = 0, stamina = 0, blocked = 0, updating_health = TRUE, required_status = null, break_modifier = 1, specialtype = DAMAGE_NONE, woundmod = 1)
	...
	//Brute wounds
	if((brute > wound_threshold) && prob((brute / 1.5) * woundmod))
		switch(specialtype)
	...
	//Burn wounds
	if((burn > wound_threshold) && prob((burn / 1.5) * woundmod))
		switch(specialtype)
...
/obj/item/bodypart/proc/on_mob_move()
	...
	if(bone_status != BONE_FLAG_BROKEN || status == BODYPART_ROBOTIC || bone_status == BONE_FLAG_SPLINTED || !owner)
		return
