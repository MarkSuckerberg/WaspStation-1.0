...
/mob/living/carbon/human/get_status_tab_items()
	...
	*/
	...
	if(B && B.controlling)
		...
		. += "Chemicals: [B.chemicals]"								//Wasp End
		...
		if(hud_used.healthdoll)
			...
			if(stat != DEAD)
				...
				for(var/t in get_broken_limbs()) //Disabled limbs
					hud_used.healthdoll.add_overlay(mutable_appearance('icons/mob/screen_gen.dmi', "[t]_fract"))
			else
...
/mob/living/carbon/human/updatehealth()
	...
	if(HAS_TRAIT(src, TRAIT_IGNOREDAMAGESLOWDOWN))
		...
		return
	var/health_deficiency = (maxHealth - health + staminaloss)		// Waspstation Edit - Stamina and health damage stack
